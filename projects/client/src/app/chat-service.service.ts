...
export class ChatServiceService {
  ...
  initializeWebSocketConnection(senderEmail:string) {
    const serverUrl = 'https://app-250220182315.azurewebsites.net/websocket';
  ...
  getMessages(senderEmail:string, receiverEmail:string):Observable<ChatMessage[]>{
    return this.http.get<ChatMessage[]>(`https://app-250220182315.azurewebsites.net/reds/chatMessages/${senderEmail}/${receiverEmail}`);
  }
  sendTranslation(text:any){
    return this.http.post(`https://app-250220182315.azurewebsites.net/translator/translate`,text);
  }
